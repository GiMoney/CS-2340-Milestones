...
public class Game extends ViewController {
    ...
    public static void main(String[] args) {
        ...
        int q = 0;
        Region currRegion = region.get(q);
        int currX = currRegion.getX();
        int currY = currRegion.getY();
        while(q < 10) {
            String name = region.get(q).getName();
            int newX = region.get(q).getX();
            int newY = region.get(q).getY();
            int distance = (int)Math.sqrt(Math.pow((currX - newX), 2) + Math.pow((currY - newY), 2));
            JButton rec = new JButton(name + "(" + region.get(q).getX() + "," + region.get(q).getY() + ")" + "distance away: " + distance);
            rec.setLayout(null);
